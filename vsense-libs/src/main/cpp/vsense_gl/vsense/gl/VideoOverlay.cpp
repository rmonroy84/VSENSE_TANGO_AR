...
VideoOverlay::VideoOverlay(AAssetManager* assetManager)
...
void VideoOverlay::initialize(AAssetManager* assetManager) {
  AAsset* vertexShaderAsset = AAssetManager_open(assetManager, "shaders/videoOverlay.vert", AASSET_MODE_BUFFER);
  ...
  fragmentShaderAsset = AAssetManager_open(assetManager, "shaders/videoOverlay.frag", AASSET_MODE_BUFFER);
...
void VideoOverlay::setDisplayRotation(TangoSupportRotation displayRotation) {
...
void VideoOverlay::setTextureOffset(float screenWidth, float screenHeight, float imageWidth, float imageHeight) {
...
void VideoOverlay::render(const glm::mat4& viewMat, const glm::mat4& projMat) {
  if(!visible_)
    return;
